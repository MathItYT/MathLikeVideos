...
# from manimlib.mobject.svg.old_tex_mobject import Tex, TexText
...
class SphereExample(InteractiveScene):
    ...
    def construct(self):
        ...
        # Create the sphere
        img_path = "out/images/PreviewThreeExamples.png"
        radius = 2.5
        ...
        banner = TexturedSurface(
            Surface(resolution=sphere.resolution), img_path)
        ...
        slices.scale(radius)
        slice_highlights = slices[0][len(
            slices[0]) // 4:3 * len(slices[0]) // 4].copy().set_color(YELLOW)
        ...
        if self.show_true_slices:
            ...
            curve = ParametricCurve(
                lambda phi: [-math.sin(phi) * dtheta / 2, PI / 2 - phi, 0], t_range=(0, PI / 2, 0.1))
        ...
        arc_tex = "{\\pi \\over 2} R"
        circ_tex = "2\\pi R"
        eq_parts = ["\\text{Area}", "=", arc_tex,
                    "\\times", circ_tex, "=", "\\pi^2 R^2"]
...
class Proof2Slide(InteractiveScene):
    def construct(self):
        ...
        v_lines = VGroup(*(
            DashedLine(ORIGIN, circle.get_height()
                       * UP).move_to(nl.n2p(x), DOWN)
            for x in range(5)
        ))
        ...
        def update_circum(circum):
            ...
            arc = Arc(start_angle=-TAU / 4, angle=TAU *
                      (1 - get_t()), radius=radius)
        ...
        radial_line = Line(circle.get_bottom(),
                           circle.get_center(), stroke_width=1)
        radial_line.add_updater(lambda m: m.set_angle(
            TAU * (0.75 - get_t())).shift(circle.get_center() - m.get_start()))
...
class CircleExample(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(TransformFromCopy(
            width_line, semi_circ, path_arc=-PI / 2, run_time=2))
        self.wait()
        ...
        self.play(
            TransformMatchingShapes(
                VGroup(height_form, width_form).copy(), rhs)
        )
        self.wait()
        self.play(ShowCreation(rect))
    ...
    def get_sectors(self, circle, n_slices=20, fill_colors=[BLUE_D, BLUE_E]):
        angle = TAU / n_slices
        sectors = VGroup(*(
            Sector(angle=angle, start_angle=i * angle,
...
class SphereSectorAnalysis(Scene):
    ...
    def construct(self):
        ...
        width_label.add_updater(lambda m: m.next_to(
            width_line, OUT, buff=SMALL_BUFF).set_width(1.5 * width_line.get_width() + 1e-2))
        ...
        def get_angle_label():
            arc = Arc(start_angle=90 * DEGREES, angle=-
                      get_phi(), radius=0.5, n_components=8)
        ...
        angle_label[0].add_updater(lambda m: m.match_points(
            get_angle_label()[0]).set_stroke(WHITE, 2).set_fill(opacity=0))
        angle_label[1].add_updater(lambda m: m.move_to(get_angle_label()[1]))
        lat_line_radius = get_lat_line_radius()
        lat_line_radius.add_updater(
            lambda m: m.match_points(get_lat_line_radius()))
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(angle_label, lambda m,
                                a: m.update().set_opacity(a)),
        )
        self.wait()
        self.play(
        ...
        lat_line_label.add_updater(lambda m: m.next_to(
            lat_line_radius, OUT, SMALL_BUFF))
        self.play(
        ...
        # Show delta theta
        brace = Brace(Line(ORIGIN, radius * dtheta * RIGHT),
                      UP, buff=SMALL_BUFF)
        ...
        # Exact formula
        formula1 = Tex("2\\pi", " R \\sin(\\phi)", "\\cdot",
                       "{\\Delta \\theta", " \\over 2\\pi}")
        ...
        axes.y_axis.add(Text("Wedge width", font_size=24,
                        color=RED).next_to(axes.c2p(0, 1), LEFT))
        axes.x_axis.add(
            Tex("\\pi / 2", font_size=24).next_to(axes.c2p(PI / 2, 0), DOWN))
        axes.x_axis.add(Tex("\\pi", font_size=24).next_to(
            axes.c2p(PI, 0), DOWN))
        axes.x_axis.add(Tex("\\phi", font_size=24).next_to(
            axes.c2p(PI, 0), UR, buff=SMALL_BUFF))
        axes.to_corner(UL)
        axes.fix_in_frame()
        ...
        self.play(
            ...
            ShowCreation(graph),
            UpdateFromAlphaFunc(
                dot, lambda d, a: d.move_to(graph_copy.pfp(a))),
            run_time=6,
        )
        self.wait()
        self.play(
            ...
            UpdateFromAlphaFunc(
                dot, lambda d, a: d.move_to(graph.pfp(1 - 0.5 * a))),
            run_time=3,
        )
        self.wait()
...
class FakeAreaManipulation(InteractiveScene):
    ...
    def construct(self):
        ...
        lil_lines = VGroup(
            *(Line(tri.get_corner(DL), tri.get_corner(UR)) for tri in tris))
    ...
    def get_diagrams(self):
        ...
        big_tris = VGroup(big_tri, big_tri.copy().rotate(
            PI, UP, about_point=ORIGIN))
...
class ContrastSphericalGeometry(InteractiveScene):
    def construct(self):
        ...
        # Setup sphere
        sphere = TexturedSurface(
            Sphere(radius=1.5), "EarthTextureMap", "NightEarthTextureMap")
        mesh = SurfaceMesh(sphere)
        ...
        pre_tri_arcs, pre_tri_angle_labels = self.get_flat_angle_labels(
            pre_tri)
        ...
        sph_tri = VGroup(
            ...
            VMobject().pointwise_become_partial(
                mesh[19], 0.5, 1.0).reverse_points(),
            VMobject().pointwise_become_partial(
                mesh[26], 16 / 20, 19 / 20).reverse_points(),
        )
        ...
        for label, curve in zip(sph_tri_angle_labels, sph_tri):
            ...
            label.add_updater(lambda l: l.shift(
                0.2 * normalize(sph_tri.get_center() - l.curve.get_end())))
        ...
        unwrapped = TexturedSurface(
            Surface(), "EarthTextureMap", "NightEarthTextureMap")
    ...
    def get_flat_angle_labels(self, tri):
        ...
        for label, arc in zip(labels, arcs):
            vect = normalize(
                arc.pfp(0.5) - midpoint(arc.get_start(), arc.get_end()))
    ...
    def get_aligned_angle_labels(self, tri, arcs, labels):
        ...
        VGroup(new_arcs[0], new_labels[0]).rotate(
            PI, about_point=midpoint(verts[1], verts[2]))
        VGroup(new_arcs[1], new_labels[1]).rotate(
            PI, about_point=midpoint(verts[1], verts[0]))
...
class SquareCircleExample(InteractiveScene):
    def construct(self):
        ...
        points = [square.get_edge_center(np.round(vect))
        ...
        self.wait()
        self.play(
            TransformFromCopy(VGroup(radial_line), new_radii,
        ...
        for i, curve in enumerate(approx_curves[1:]):
            ...
            if i == 0:
                ...
                self.play(FadeOut(dots), FadeOut(sc), FadeOut(cc), VGroup(
                    square, curve).animate.set_stroke(opacity=1))
        ...
        self.play(frame.animate.set_height(
            0.05).move_to(circle.pfp(1 / 8)), run_time=4)
        self.wait()
        ...
        c_labels = VGroup(*(Tex(f"c_{n}(t)")
                          for n in range(len(approx_curves))))
        ...
        # Write limits
        lim_tex_ex = Tex(
            "\\lim_{n \\to \\infty} c_{n}(" + "{:.1f}".format(get_t()) + ")")
        lim_tex = Tex("c_\\infty(t)", ":=",
                      "\\lim_{n \\to \\infty} c_{n}(t)")
        for lt in lim_tex_ex, lim_tex:
            lt.next_to(t_axis, DOWN, aligned_edge=LEFT, buff=2.0)
        lim_arrow = Arrow(lim_tex_ex.get_corner(
            UL), dot.get_center(), buff=0.1, stroke_width=2, color=YELLOW)
        ...
        self.wait()
        self.play(FadeOut(dot_shadows), FadeOut(lim_arrow),
        ...
        lim_len = Tex(
            "\\lim_{n \\to \\infty}\\Big(\\text{len}\\big(c_n(t)\\big) \\Big) = 8", tex_to_color_map=t2c)
        len_lim = Tex(
            "\\text{len} \\Big( \\lim_{n \\to \\infty} c_n(t) \\Big) = 2\\pi", tex_to_color_map=t2c)
        ...
        self.wait()
        self.play(
            VGroup(lim_tex, lim_len).animate.next_to(
                frame.get_corner(UR), DL, MED_LARGE_BUFF),
        ...
        self.play(*(
            TransformFromCopy(lim_len.select_parts(tex),
                              len_lim.select_parts(tex))
            for tex in t2c.keys()
        ))
    ...
    def get_square_approx(self, circle, n_samples):
        ...
        for p1, p2 in zip(points, points[1:]):
            ...
            corner = corners[np.argmax(
                np.apply_along_axis(np.linalg.norm, 1, corners))]
...
class UpshotOfLimitExample(InteractiveScene):
    def construct(self):
        ...
        circle_example = Group(
            Group(*(
                ImageMobject(os.path.join(
                    folder, "SquareApprox" + end)).set_width(1.25)
        ...
        fourier_example = Group(
            Group(*(
                ImageMobject(os.path.join(
                    folder, "Fourier" + end)).set_width(1.25)
                for end in ["1", "2", "3", "4", "Inf"]
            )),
            VGroup(
...
class Proof3Slide(InteractiveScene):
    def construct(self):
        ...
        subtitle = Text("All triangles are isosceles",
...
class IntegralExample(InteractiveScene):
    def construct(self):
        ...
        all_rects = VGroup(*(
            axes.get_riemann_rectangles(graph, (0, 3), dx).set_stroke(
                BLACK, np.round(4 * dx, 1), background=False)
        ...
        self.play(frame.animate.move_to(
            graph.pfp(0.5)).set_height(0.1), run_time=3)
        self.wait()
        self.play(frame.animate.to_default_state(), run_time=3)
        self.wait()
...
class IntegralError(InteractiveScene):
    def construct(self):
        ...
        error_arrows = VGroup(*(
            Arrow(error_label, rect.get_top() + 0.2 *
                  UP, stroke_width=2, tip_width_ratio=8)
            for rect in rects
        ))
...
class FalseEuclidProofAnnotation(InteractiveScene):
    def construct(self):
        ...
        self.play(
            AB.animate.set_angle(-90 * DEGREES).next_to(A,
                                                        RIGHT, aligned_edge=UP, buff=2),
            AC.animate.set_angle(-90 * DEGREES).next_to(A,
                                                        RIGHT, aligned_edge=UP, buff=2.5),
        )
        self.wait()
        self.play(FadeOut(AB), FadeOut(AC))

        # Bisector labels
        perp = Text("Perpendicular\nbisector", font_size=30,
                    color=BLACK, stroke_width=0)
...
class FalseEuclidFollowup(InteractiveScene):
    def construct(self):
        ...
        tri3 = tri2.copy().set_height(math.sqrt(3) * tri2.get_width() /
                                      2, stretch=True, about_edge=DOWN)
        ...
        for letter, vert in zip(labels, tri.get_vertices()):
            letter.next_to(vert, normalize(
                vert - tri.get_center_of_mass()), SMALL_BUFF)
...
class SideSumTruthiness(InteractiveScene):
    def construct(self):
        ...
        AE.add_updater(lambda l: l.put_start_and_end_on(
            A.get_center(), E.get_center()))
        EC.add_updater(lambda l: l.put_start_and_end_on(
            E.get_center(), C.get_center()))
        ...
        # Equation
        eq = Tex(
            "\\overline{AE} + \\overline{EC} = \\overline{AC}", font_size=60)
        eq.to_corner(UR)
        ...
        labels[0].add_updater(lambda m: m.set_opacity(
            1 if 0 < alpha_tracker.get_value() < 1 else 0))
        labels[1].add_updater(lambda m: m.set_opacity(
            0 if 0 < alpha_tracker.get_value() < 1 else 1))
...
class PythagoreanProofSketch(InteractiveScene):
    def construct(self):
        ...
        verts = tri.get_vertices()
        tris = VGroup(tri, tri.copy().rotate(
            PI, about_point=midpoint(*verts[1:])))
        tris.add(*tris.copy().rotate(PI, axis=UL,
                 about_point=tris.get_corner(DL)))
        tris[2:4].flip(UP)
        ...
        self.play(
            ...
            side_labels.animate.scale(
                scale_factor, about_point=pre_tri.get_center()).shift(shift_vect)
        )
        ...
        # Final gif
        self.play(
            Transform(tris, tris.saved_state, lag_ratio=0.2,
                      run_time=3, path_arc=45 * DEGREES),
        ...
        self.wait()
        self.play(
            Transform(tris, tris2, lag_ratio=0.2,
                      run_time=3, path_arc=45 * DEGREES),
